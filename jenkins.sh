...
if [ -d "skeleton" ]; then
    ...
    for layers in "('basic',)" "('smart', 'basic')" "('web', 'basic')"; do
        ...
        # Bug in django-setuptest means database org must exist, even
        # though it is not used.
        psql -U test -d template1 -c "CREATE DATABASE org WITH OWNER test ENCODING 'UTF8' TEMPLATE template_postgis;"
        psql -U test -d template1 -c "DROP DATABASE test_org;"
        ./bin/setuptest-runner setup.py test
        EXIT_CODE=$?
        if [ $EXIT_CODE != 0 ]; then
            ONE_LAYER_FAILED=1
